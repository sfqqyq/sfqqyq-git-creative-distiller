...
def build_image_prompt(point: CreativePoint) -> str:
    ...
    if scenarios:
        first = scenarios[0]
        scenario_text = chinese_label_text(f"{first.get('name', '')}：{first.get('description', '')}")
    title = chinese_label_text(point.title)

    prompt = f"""
生成一张单幅横版卡通场景插画，风格轻松、活泼、明亮，像儿童绘本和产品科普插画。
只画一个完整场景，不要海报、不要信息图、不要流程图、不要架构图、不要漫画分镜、不要箭头、不要标题栏、不要对话气泡。
画面里禁止出现任何文字、字母、数字、拼音、代码、文档段落、表格、水印、标识、屏幕字幕。屏幕和卡片只能画成简单图标或色块，不能有字。

画面主题：{title}
参考场景：{scenario_text}

画面内容：明亮办公室里，一位工作人员面对一团乱线和许多彩色任务卡片有点困扰；旁边一个可爱的圆形机器人助手，把任务卡片放进几个透明小盒子里分别处理，桌面逐渐变整洁。用人物表情、颜色和动作表现“原来混乱，现在清楚、省心、可靠”。
构图要求：16:9，主体明确，人物表情友好，色彩温暖，背景简洁，不要深色压抑背景。
""".strip()
